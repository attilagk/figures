...
{\(G_\mathrm{h}\)
: Bayesian network
%\\\(X^v \perp X^{\mathrm{nonde}(v)} \; | \; X^{\mathrm{pa}(v)}\)
}
(V1.east|-S1)
;
...
{\(G_\mathrm{p}\)
: Markov network
%\\\(X_i \perp X_{\mathrm{nonne}(i)} \; | \; X_{\mathrm{ne}(i)} \)
}
(A10|-C3c)
;
...
\draw[decorate, decoration={brace, raise=10 pt}]
(S10.north) --
node[right=15 pt, align=left] {\(V_\mathrm{p}\)}
(S10.south)
;
...
(m0) +(0,10 pt) node[anchor=south west] {\(X\)}
;

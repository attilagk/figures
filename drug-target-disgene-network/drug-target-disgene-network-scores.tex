...
\foreach \i/\h/\r in {0/0.8/4,1/0.8/4,2/0.9/3,3/1.6/1,4/1.3/2} {
	\coordinate[] (f\i) at (\i * 0.75,1.5);
	\node[yshift=\h cm,node cs:name=a\i,anchor=south] (g\i) at (a\i) {(\r )};
	\draw[line width=5pt] (f\i) -- (g\i);
}

\draw[->] (-0.5,1.5) -- node[rotate=0,anchor=east,align=center]{\small
	proximity\\score\\(rank)} (-0.5, 3.0);
...
\node[anchor=north,red] (AD genes) at (1.5,-2.25) {AD risk genes};
